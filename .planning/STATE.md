...
Status: Decision needed (GHCR vs ECR — GHCR works today, ECR needs OIDC setup)
...
- ECR over GHCR: Client already uses ECR, lower latency — **decision still open** (GHCR works today, ECR needs Phase 2)
...
3. **Image registry:** GHCR (works now) vs ECR (Phase 2 required)
